...
#' @param cf.depth.grid How deep should trees be grown in CIF (Inf is full depth, default from partykit).
...
#' @param xgb.max.depth.grid Maximum depth of trees for XGBoost.
  ...
  if (verbose) {
    ...
    for (ml in ML) {
      if (ml == "Lasso") {
        ...
        n_combinations <- length(xgb.nrounds.grid) * length(xgb.max.depth.grid)
